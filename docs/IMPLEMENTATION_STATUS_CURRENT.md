...
## Dual login update — 2026-09-10

Google access approval is implemented via migration `0004_access_requests.sql`, verified-Google pending request capture, Owner/Director Settings review, transactional user creation/review/audit and normalized email uniqueness. No public signup or automatic user creation occurs before approval. Existing inactive users remain blocked. See `docs/DUAL_LOGIN.md` for migration and review operation.

Migration 0004 was applied to the configured database. Verification passed: typecheck, production build, DB smoke (41 tables / 7 views), static verification and auth regressions. Rollback-only database checks covered pending deduplication, Owner/Director approval without passwords, rejection, unauthorized roles, Google identity resolution and normalized email uniqueness. Real browser Google consent/approval UAT remains outstanding.

Google OAuth is retained. Credentials login now uses bcryptjs and the existing users/RBAC, with five-attempt/15-minute lockout, protected password changes, password-session invalidation and interactive operator password setup. Migration `0003_dual_login.sql` adds the five password-related fields. See [DUAL_LOGIN.md](DUAL_LOGIN.md) for deployment and verification; live Google/browser UAT remains a separate release gate.

Verified: typecheck, production build, DB smoke (40 tables / 7 views plus password schema), static verification, auth unit/regression checks, and the real password service against Neon with all synthetic data rolled back. HTTP checks confirmed both login providers, the login form and anonymous change-password redirect. Interactive CLI input was hidden and cancellation performed no write. Migration 0003 was applied to the configured database; existing user passwords were not changed. Installation reported four moderate dependency advisories; no automatic dependency upgrades were applied.
