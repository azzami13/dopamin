...
export default async function SalesPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  return <main className="page-shell">
    ...
    <section className="dashboard-grid">
      <article className="panel"><h2>Top Menu</h2><div className="table-wrap"><table className="data-table"><thead><tr><th>Menu</th><th>Category</th><th>Qty</th><th className="money-cell">Revenue</th></tr></thead><tbody>{(data.topItems as Array<Record<string, unknown>>).map((r) => <tr key={String(r.code)}><td>{String(r.name)}<br/><small>{String(r.code)}</small></td><td>{String(r.category)}</td><td>{String(r.quantity)}</td><td className="money-cell">{idr(r.revenue)}</td></tr>)}</tbody></table></div></article>
