...
export function PurchaseCreateForm({ defaultDate }: { defaultDate: string }) {
  ...
  return <form className="panel form-stack" onSubmit={submit}>
    ...
    {items.map((item,i)=><div className="line-form-grid" key={i}>
      <input placeholder="Nama item" value={item.itemName} onChange={(e)=>setItem(i,"itemName",e.target.value)} required />
      <input placeholder="Qty" inputMode="decimal" value={item.quantity} onChange={(e)=>setItem(i,"quantity",e.target.value)} required />
      <input placeholder="Unit" value={item.unit} onChange={(e)=>setItem(i,"unit",e.target.value)} required />
      <input placeholder="Estimasi/unit" inputMode="numeric" value={item.estimatedUnitCost} onChange={(e)=>setItem(i,"estimatedUnitCost",e.target.value)} required />
