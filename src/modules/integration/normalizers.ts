...
async function mappingsFor(dataSourceId: string): Promise<MappingRow[]> {
  return db.select({
...
export async function normalizeCashier(input: { dataSourceId: string; rawSubmissionId: string; envelope: GoogleFormEnvelope }): Promise<NormalizeResult> {
  const mappings = await mappingsFor(input.dataSourceId);
  ...
  if (!businessDate) {
    await issue(db, input.rawSubmissionId, "CASHIER", "CRITICAL", "INVALID_BUSINESS_DATE", "Tanggal Pelaporan wajib ada dan harus berupa tanggal yang valid.", dateSource?.sourceField);
  ...
  return db.transaction(async (tx) => {
    ...
    const [report] = await tx.insert(cashierReports).values({
      ...
      reportStatus: "VALID",
    }).returning({ id: cashierReports.id });
...
export async function normalizeSales(input: { source: "KITCHEN" | "BEVERAGE"; dataSourceId: string; rawSubmissionId: string; envelope: GoogleFormEnvelope }): Promise<NormalizeResult> {
  const mappings = await mappingsFor(input.dataSourceId);
  const payload = input.envelope.payload;
  const mappedDate = mappedOne(payload, mappings, "BUSINESS_DATE");
  ...
  const businessDate = parseBusinessDate(dateSource?.value);
  if (!businessDate) {
    await issue(db, input.rawSubmissionId, input.source, "CRITICAL", "INVALID_BUSINESS_DATE", input.source === "BEVERAGE" ? "Beverage wajib memiliki Tanggal Penjualan eksplisit sebelum dianggap final." : "Tanggal penjualan wajib ada dan valid.", dateSource?.sourceField);
    return { status: "NEEDS_REVIEW", businessDate: null, issueCount: 1 };
  }
  ...
  return db.transaction(async (tx) => {
    ...
    const [report] = await tx.insert(salesReports).values({
      sourceSubmissionId: input.rawSubmissionId,
      inputterUserId,
    ...
    for (const item of itemMaps) {
      ...
      const [menu] = await tx.select({ id: menuItems.id, category: menuItems.category }).from(menuItems).where(eq(menuItems.code, item.targetKey)).limit(1);
